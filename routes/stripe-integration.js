...
async function handleCheckoutCompleted(session) {
  ...
  const isDemoPaymentsDisabled = process.env.DEMO_PAYMENTS_DISABLED === 'true' || process.env.DEMO_PAYMENTS_DISABLED === '1';

  // Mode démo : on ne marque jamais la commande comme payée (les clients peuvent accéder à Stripe mais aucun paiement réel)
  if (isDemoPaymentsDisabled && order_id) {
    console.log(`[Démo] Checkout Stripe terminé pour commande ${order_number || order_id} — non marquée comme payée`);
    return;
  }
