...
const { requireAdmin, requireAuth } = require('../middleware/auths');
...
const profileStorage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    const ext = (path.extname(file.originalname) || '.jpg').toLowerCase().replace(/jpe?g/, 'jpg');
    const name = `profile-${req.userId}-${Date.now()}${ext}`;
    cb(null, name);
  },
});
...
// POST /upload/profile-image — photo de profil (utilisateur connecté)
router.post(
  '/profile-image',
  requireAuth,
  ...
  (req, res) => {
    ...
    const baseUrl = process.env.API_PUBLIC_URL || `${req.protocol}://${req.get('host')}`;
    const url = `${baseUrl.replace(/\/$/, '')}/uploads/profiles/${req.file.filename}`;
    return res.json({ success: true, url });
  },
  (err, _req, res, _next) => {
    if (err instanceof multer.MulterError) {
